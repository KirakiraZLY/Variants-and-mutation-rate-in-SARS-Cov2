...
boxplot(covid_specific$delta_N~covid_specific$pangolin_lineage,
  ylim = c(-50,50),
  ylab = "delta_N",
  main = "delta_N by linegages")
```
...
test.stat1 <- abs(
  mean(covid_specific$delta_N[covid_specific$VarShort == "VOC Omicron GRA"]) -
    mean(covid_specific$delta_N[covid_specific$VarShort == "VOC Delta GK"]))
test.stat1
...
variable <- covid_specific$delta_N
...
for (i in 1:B) {
  Boot.test.stat1[i] <- abs(mean(BootstrapSample[1:n/2,i]) -
                              mean(BootstrapSample[floor(n/2+1):n,i]))
  
  Boot.test.stat2[i] <- abs(median(BootstrapSample[1:n/2,i]) -
