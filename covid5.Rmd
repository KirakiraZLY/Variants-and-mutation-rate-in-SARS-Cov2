...
covid_specific_1 <- covid_specific[covid_specific$pangolin_lineage == c("BF.7"),]
...
sumstatz <- data.frame(whichstat = c("mean",
                       ...
                       value     = c(mean(slope_NS$date),
                                     mean(slope_NS$date)+2*sd(slope_NS$date),
                                     mean(slope_NS$date)-2*sd(slope_NS$date)))

ggplot(slope_NS, aes(x = date)) +
  ...
  labs(title = "dnds BQ.1.1 boots = 500") +
  xlim(-5,5)
