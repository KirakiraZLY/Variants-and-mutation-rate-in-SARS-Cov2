...
covid3_ba5 <- covid3_ba5 %>% filter(aa_change == "R346T") %>% filter(year(date) >= 2021)
...
```{r}
covid_2 <- covid_2 %>% arrange(date)
delta_N <- diff(covid_2$count_N)
delta_N <- c(0,delta_N)
covid_2$delta_N <- delta_N

delta_S <- diff(covid_2$count_S)
delta_S <- c(0,delta_S)
covid_2$delta_S <- delta_S

covid_2$dnds <- delta_N/delta_S
...
```{r}
ggplot(covid_2, aes(x = date, y = dnds, col = VarShort)) +
  geom_point()
```
