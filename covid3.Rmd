...
Finding relative growth advantage between BA.5 and BA.2
```{r}
...
```{r}
covid3_ba5 %>% filter(aa_change == "K356T")
tb1 <- table(covid3_ba5$date)
tb2 <- table(covid_2$date)
tb1 <- data.frame(tb1)
tb2 <- data.frame(tb2)
names(tb1) <- c("date","num1_K356T")
names(tb2) <- c("date","num2")
covid4_ba5 <- merge(tb1,tb2,by = "date")
covid4_ba5["Freq"] <- covid4_ba5$num1_K356T / covid4_ba5$num2
ggplot(data = covid4_ba5, mapping = aes(x = date, y = Freq)) +
  geom_point() +
  labs(title = "K356T, BA.5") +
  scale_x_discrete(" ")

```

```{r}
covid3_ba2 %>% filter(aa_change == "K356T")
...
covid4_ba2["Freq"] <- covid4_ba2$num1_K356T / covid4_ba2$num2
ggplot(data = covid4_ba2, mapping = aes(x = date, y = Freq)) +
  geom_point() +
  labs(title = "K356T, BA.2") +
  scale_x_discrete(" ")
```
